...
{
    memcpy(&tpm_token, _token, sizeof(struct token));
    tpm_token.pos = lex_file_position();
    if (lex_is_in_expression())
    {
        tpm_token.between_brackets = buffer_ptr(lex_process->parentheses_buffer);
    }
    return &tpm_token;
}
...
static void lex_new_epression()
{
    lex_process->current_expression_count++;
    if (lex_process->curent_expression_count == 1)
    {
...
static void lex_finish_expression()
{
    ...
    {
        compiler_error(lex_process->compiler, "Unexpected closing parentheses\n");
    }
}
bool lex_is_in_expression()
{
...
{
    struct buffer *buffer = buffer_create();
    char c = 0 : LEX_GETC_IF(buffer, c, c != '\n' && c != EOF);
    return token_creat(&(struct token){.type = TOKEN_TYPE_COMMENT, .sval = buffer_ptr(buffer)});
}

struct token *token_make_multipleline_comment()
{
    struct buffer *buffer = buffer_create();
    char c = 0;
    while (1)
...
struct token *handle_comment()
{
    ...
    if (c == '/')
    {
        ...
        {
            nextc();
            return token_make_multipleline_comment();
        }
...
static struct token *token_make_symbol()
{
    ...
    sturct token *token = token_create(&(struct token){.type = TOKEN_TYPE_SYMBOL, .cval = c});
    return token;
}
struct struct token *token_make_identifier_or_keyword()
{
    struct buffer *buffer = buffer_create();
    char c = 0;
...
struct token *read_special_tokem()
{
    char c = peekc();
    if (isalpha(c) || c == '_')
    {
...
struct token *token_make_especial_number()
{
    struct token *token = NULL;
    struct token *last_token = lexer_last_token();
    if (!last_token || !(last_token->type == TOKEN_TYPE_OPERATOR && last_token->llnum == '0'))
    {
        return token_make_identifier_or_keyword();
    }
...
struct token *token_make_quote()
{
    ...
    if (nextc() != '\'')
    {
        compiler_error(lex_process->compiler, "Invalid character\n");
    }
...
struct token *read_next_token()
{
    ...
    case 'b':
    case 'x':
        token = token_make_especial_number();
        break;
    case '"':
    ...
    default:
        ...
        if (!token)
        {
            compiler_error(lex_process->compiler, "Unknow character\n");
        }
    }
    return token;
}
int lex(struct lex_process *process)
{
    ...
    while (token)
    {
        vetor_push(process->token_vec, token);
...
void lexer_string_buffer_push_char(struct lex_process* process)
{
    struct buffer* buf = lex_process_private(process);
    buffer_push(buf, c);
}
...
struct lex_process* tokens_build_for_string(struct compile_process* compiler, const char* str)
{
    ...
    if (lex(lex_process) != LEZICAL_ANALYSIS_ALL_OK)
    {
        return NULL;
    }
    return lex_process;
